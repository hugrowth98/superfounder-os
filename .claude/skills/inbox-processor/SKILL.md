...
- Transcript de call -> deplacer vers `Meeting/Coaching/<Nom>/` ou `Meeting/Clients/<Nom>/` si la personne a un dossier, sinon `Meeting/Prospects/YYYY-MM/` (regle complete dans `Meeting/Meeting.md`). Autre source brute -> le `sources/` du dossier concerne.
