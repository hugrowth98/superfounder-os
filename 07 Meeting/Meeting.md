...
Ne va pas ici : le compte rendu envoyé au client après le call (il va dans `06 Clients/<Client>/`), les notes de réunion sans transcript.

## Cadre

Nommage `Transcript_Sujet_YYYY-MM-DD.md`. Routage d'un nouveau transcript, premier match gagne :

1. Call avec un client signé : `Clients/<Nom>/`, le même nom que dans `06 Clients/`.
...
Un prospect devient client à la signature. On crée alors `Clients/<Nom>/` et on y remonte tous ses calls de `Prospects/` : l'historique reste ensemble. Quand une mission se termine, le dossier part dans `11 Archives/` tel quel.

Ce qui se décide dans un call ruisselle vers la note du client via `/done`. Les douleurs entendues alimentent `02 Contexte/Clients-Problems-and-Messages.md` (proposé en diff) et vos idées de contenu.
