...
d'ouvrir `Vente/` dans Claude Code et veut demarrer. C'est un dirigeant ou un commercial, pas
...
Le dossier courant est `Vente/`. La racine du workspace (un niveau au-dessus)
...
Tout ce qui est technique (`.env`, skills, livrables) reste dans `Vente/`.
