...
- Un skill = un dossier avec un `SKILL.md` et, si besoin, `references/` (fichiers lus à la demande) et `scripts/` (code exécuté par le skill). Tous les skills vont dans `10 Skills/`, un dossier par skill. `.claude/skills` et `.agents/skills` sont des liens vers `10 Skills/`, on n'y écrit jamais.
- Un skill ne contient jamais de donnée sur l'utilisateur : il lit `02 Contexte/`, `05 Departements/Go-to-Market/contexte.md` et `05 Departements/Contenu/LinkedIn/` à l'exécution. Pas de jetons de substitution.
