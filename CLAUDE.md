...
| `Meeting/` | Les transcripts de calls, rangés par relation : `Clients/<Nom>/`, `Coaching/<Nom>/`, `Anciens-clients/`, `Ateliers-cold-call/`, `Ateliers-collectifs/`, `Events/`, `Prospects/`, `Interne/`, `Autres/`. Ce qui s'y décide ruisselle vers la note du client. | Le compte rendu envoyé (il va chez le client) |
...
|---|---|
| Transcript de call | `Meeting/Coaching/<Nom>/` ou `Meeting/Clients/<Nom>/` si la personne a un dossier, sinon `Ateliers-cold-call/`, `Ateliers-collectifs/`, `Events/`, `Prospects/`, `Interne/` ou `Autres/` (règle dans `Meeting/Meeting.md`), puis les décisions ruissellent vers la note du client |
