...
Skills/            les 46 skills, une bibliothèque unique ; .claude/skills et .agents/skills pointent dessus
```
...
La structure est livrée avec Superfounder OS. Vérifie simplement qu'elle est intacte (`ls`) : `About-Me/`, `Contexte/` (7 fichiers), `Branding/`, `Ressources/templates/`, `Inbox/`, `Journal/`, `Veille/`, `Meeting/`, `Produit-Client/`, `Marketing/`, `Vente/`, `Strategie/`, `Archives/`, `Skills/`. Chaque dossier de travail a une note du même nom (`Vente/Vente.md`).

Vérifie que `.claude/skills` et `.agents/skills` sont des liens symboliques vers `Skills/` (`ls -l`). Si ce sont des fichiers texte (Windows cloné sans mode développeur), remplace-les par des copies de `Skills/` et dis-le à l'utilisateur.
...
4. Créer `Skills/[nom-skill]/SKILL.md` en suivant la convention frontmatter (name, description, argument-hint, user-invocable, context).
