...
1. **Le second cerveau, en premier** : `About-Me/` (déjà chargé), `Contexte/Offer-Positioning.md`, `Contexte/Clients-Problems-and-Messages.md`, `Contexte/Tone-and-Voice.md`, puis `Marketing/LinkedIn/ressources/strategie-contenu.md` (convictions, piliers) et `posts-de-reference.md`. Dans Superfounder OS, c'est là que vit le positionnement : pas besoin de le chercher ailleurs.
...
4. **Banque existante** : `Marketing/LinkedIn/ressources/banque-vecu.md`, créée par une session précédente de ce skill (voir Phase 4).
...
Note ces réponses dans la fiche de calibrage. À la fin de session, propose de les verser dans `Contexte/` ou `strategie-contenu.md` (Phase 4) pour ne jamais les redemander.
...
Un seul fichier : `Marketing/LinkedIn/ressources/banque-vecu.md`, toutes les pépites, en append. Crée-le à la première session avec un en-tête qui explique le format.

La fiche de calibrage (métier, cible, voix, convictions) n'a pas de fichier à elle : elle vit déjà dans `Contexte/` et `strategie-contenu.md`. Si l'interview révèle une conviction ou un élément de voix qui n'y est pas encore, propose de l'ajouter au bon fichier (validation avant d'écrire), au lieu de le dupliquer.
...
- Redemander à l'utilisateur des infos déjà présentes dans `Contexte/` ou `strategie-contenu.md`.
