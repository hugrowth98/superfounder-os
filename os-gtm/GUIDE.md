...
1. **Deposez le dossier `OS-GTM`** dans votre workspace (a cote de vos autres dossiers).
2. **Ouvrez le dossier `OS-GTM`** dans Claude Code (`claude` dans le terminal, depuis ce
   dossier). Verifiez que vous avez un abonnement Claude payant actif (Pro ou plus).
3. **Ecrivez simplement "installe l'OS GTM"** dans le chat (ou meme juste "bonjour" : Claude
   vous proposera de demarrer l'installation).

C'est tout. Claude va, tout seul :

- Lire le contenu de votre workspace pour comprendre votre activite (votre offre, votre
  cible, votre voix) et pre-remplir votre profil.
- Vous demander l'adresse de votre site web si elle manque d'informations, pour completer.
- Vous montrer le profil reconstitue et vous laisser le corriger avant de le figer.
...
Declenche `qualifier-liste`. Garde ce qui matche votre cible (definie dans `contexte.md`),
