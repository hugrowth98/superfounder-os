...
1. **Fichiers d'identité et de positionnement** : `CLAUDE.md`, `README*`, tout fichier contenant `about`, `profil`, `identity`, `bio`, `positionnement`, `offre`, `ICP`, `client`, `contexte`, `context`, `tone`, `voice`.
...
4. **Banque existante** : un fichier de banque de vécu déjà créé par une session précédente de ce skill (voir Phase 4).
...
Note ces réponses dans la fiche de calibrage. À la fin de session, elles seront sauvegardées dans le fichier profil (Phase 4) pour ne jamais les redemander.
...
Deux fichiers, dans un dossier dédié à la racine du répertoire de travail (par défaut `banque-vecu/`) :

- `banque-vecu/profil.md` : la fiche de calibrage (métier, cible, voix, convictions). Écrite à la première session, relue et enrichie ensuite. C'est ce qui évite de refaire l'onboarding.
- `banque-vecu/banque-vecu.md` : toutes les pépites, en append.

**À la première utilisation seulement**, demande où stocker : "Je range tes pépites dans `banque-vecu/` à la racine, ou tu préfères un autre dossier ?" Note le chemin retenu en haut de `profil.md` et respecte-le ensuite sans redemander.

Si le workspace a déjà une convention de rangement documentée (un `CLAUDE.md` avec des règles de routage par exemple), suis-la plutôt que le défaut.
...
- Redemander à l'utilisateur des infos déjà présentes dans `profil.md`.
