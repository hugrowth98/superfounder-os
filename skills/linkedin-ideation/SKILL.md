...
# LinkedIn Ideation - Système de veille & d'idéation {{CREATOR_NAME}}
...
2. **Sources de {{CREATOR_FIRSTNAME}}** : chaînes YouTube, newsletters, ressources sauvegardées
3. **Frameworks LinkedIn** : les 4A, TOFU/MOFU/BOFU, Authority-First, piliers thématiques

> Ce skill se nourrit du profil rempli via l'installeur. Si les sources ci-dessous contiennent encore des marqueurs `{{ ... }}`, lance `linkedin-system-installer` pour les remplir.
...
Lance 3-5 recherches ciblées sur les piliers de {{CREATOR_FIRSTNAME}} (voir `{{CONTENT_PILLARS}}` et `{{NICHE}}`). Adapte les requêtes au domaine de l'utilisateur.
...
1. **Pertinence** : est-ce lié aux piliers de {{CREATOR_FIRSTNAME}} ?
...
4. **Authority-First** : {{CREATOR_FIRSTNAME}} a-t-il une expérience personnelle ou un avis unique sur ce sujet ?
...
**Angle** : [L'angle unique de {{CREATOR_FIRSTNAME}} sur ce sujet - en 1-2 phrases]
...
Organise les idées en un calendrier ({{POSTING_FREQUENCY}}) respectant :
- **Répartition éditoriale** : voir `{{CONTENT_MIX}}`
...
## Sources de veille de {{CREATOR_FIRSTNAME}}
...
Consulte `references/saved-resources.md` pour la liste complète des guides et ressources bookmarkés par {{CREATOR_FIRSTNAME}}. Ces ressources sont une mine d'or pour les posts educational et lead-magnet.
...
1. **Authority-First** : ne proposer QUE des sujets sur lesquels {{CREATOR_FIRSTNAME}} a une expérience ou un avis unique
2. **Pas de generic** : "5 outils à tester" n'est intéressant que si {{CREATOR_FIRSTNAME}} les a TESTÉS
...
Lis ces fichiers AVANT de générer des idées, ils contiennent tout le contexte stratégique de {{CREATOR_FIRSTNAME}} :

- `references/content-strategy.md` - **LIS EN PREMIER.** Positionnement, monopole personnel, piliers thématiques, 4A, TOFU/MOFU/BOFU, Authority-First, convictions, voix et style. C'est le GPS de toute l'idéation.
- `references/liked-posts-inspiration.md` - Posts qui résonnent avec {{CREATOR_FIRSTNAME}}, classés par catégorie (4A) (à remplir via l'installeur). Analyse des patterns qui marchent.
- `references/my-posts-voice.md` - Tes posts réels analysés par thème + marqueurs de style, longueurs de référence, sujets à ÉVITER (à remplir via l'installeur). Utilise ce fichier pour calibrer les hooks et le ton des idées.
- `references/saved-resources.md` - Playbooks, guides et ressources sauvegardées (à remplir via l'installeur). Chaque ressource = 1-3 idées de posts potentielles.
