...
description: >
  ...
  second cerveau (01 About-Me/, 02 Contexte/) pour ne poser que les questions restantes, remplit
...
Le module vit dans `05 Departements/Vente/`. La racine du workspace
contient `01 About-Me/` et `02 Contexte/`, le second cerveau construit par "Installe mon second cerveau". Regle du workspace :
une information vit a un seul endroit. L'offre, la cible, la voix et les preuves vivent dans
`02 Contexte/`. `05 Departements/Go-to-Market/contexte.md` (ce dossier) ne les recopie jamais : il pointe dessus et ne contient
que ce qui est propre a la prospection (canaux, volumes, signaux, garde-fous, precisions).
Les skills vivent dans `10 Skills/`, le `.env` a la racine du workspace, les livrables dans `05 Departements/Vente/`.
...
- `01 About-Me/about-me.md` et `01 About-Me/my-company.md`
- `02 Contexte/Offer-Positioning.md` (offre, differenciation, preuves)
- `02 Contexte/Clients-Problems-and-Messages.md` (ICP, problemes dans leurs mots)
- `02 Contexte/Tone-and-Voice.md` (voix)
- `05 Departements/Go-to-Market/sources/` s'il contient deja de la matiere commerciale

Pour chacun des 3 fichiers de `02 Contexte/`, note s'il est rempli ou s'il contient encore
`[a remplir]`.
...
- **Un ou plusieurs sont vides** : l'utilisateur n'a pas encore installe son second cerveau. Dis-le lui simplement et
  ...
  second cas, ce que tu apprends s'ecrit dans les fichiers `02 Contexte/` concernes (la verite
  unique), apres validation, jamais dans `05 Departements/Go-to-Market/contexte.md`.
...
`05 Departements/Go-to-Market/contexte.md` a 8 sections. Les sections 1, 3 et 4 sont de purs pointeurs vers `02 Contexte/` : tu n'y
...
Uniquement si la Phase 0 a trouve des fichiers `02 Contexte/` vides et que l'utilisateur a
...
Utilise ce contenu pour rediger un premier jet des fichiers `02 Contexte/` vides
...
Prends en compte ses corrections. Quand il valide, ecris les precisions dans `05 Departements/Go-to-Market/contexte.md`
...
- Second cerveau : `02 Contexte/` lu (ou premier jet ecrit)
- Profil de prospection `05 Departements/Go-to-Market/contexte.md` : rempli et valide
...
- Tu n'ecris dans `02 Contexte/` que si un fichier est encore `[a remplir]`, et apres validation.
...
- Avant toute action a consequence reelle plus tard (envoi de messages, campagne), tu
  ...
  ecrire `05 Departements/Go-to-Market/contexte.md`, `.env` et connecter des outils : rien n'est envoye a personne.
