---
name: weekly-review
description: "Bilan de la semaine écoulée, report des tâches non cochées, intentions transverses, plan de la semaine suivante jour par jour, écrit dans 09 Journal/YYYY-Www.md. Le vendredi, ou dès qu'l'utilisateur dit qu'il veut planifier sa semaine."
user-invocable: true
context: main
---
...
Lis `01 About-Me/about-me.md` et `01 About-Me/my-company.md` pour les objectifs et le focus du moment. Lis les notes du journal de la semaine (`09 Journal/YYYY-MM-DD.md`) pour ce qui s'est passé session par session.
...
Lis `09 Journal/$PREV_WEEK.md`. Extrais toutes les lignes `- [ ]` non cochées. Ajoute les cases non cochées des sections Étapes et Où on en est des projets actifs (`04 Projets/*/*.md`). Si la weekly précédente n'existe pas, passe.

## Étape 4 : scanner l'activité

```bash
find "04 Projets" "05 Departements" "06 Clients" "00 Inbox" -name "*.md" -mtime -7 -not -name "_log.md" | grep -v superfounder-os | head -50
```
...
Rappelle le focus en cours dans `01 About-Me/my-company.md` si le brain dump s'en éloigne. C'est le moment de fermer une boucle plutôt que d'en ouvrir une.
...
Crée `09 Journal/YYYY-Www.md` depuis `08 Ressources/templates/journal-semaine.md`, remplis Transverse et chaque jour. Le brain dump n'est pas recopié : il a servi à extraire.
