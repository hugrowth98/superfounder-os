...
├── 01 About-Me/              qui vous êtes (chargé à chaque session)
│   ├── about-me.md
│   ├── my-company.md
│   └── anti-ai-voice.md
├── 02 Contexte/              votre cerveau (chargé à la demande)
...
├── 08 Ressources/            templates, exemples, docs d'outils
├── 00 Inbox/                 capture en vrac
├── 09 Journal/               un fichier par jour, écrit par /done
├── 08 Ressources/Veille/                sources brutes et wiki de connaissance
├── 07 Meeting/               transcripts de calls
├── 06 Clients/        un dossier par client et par produit
├── 05 Departements/Contenu/             ce qui fait venir : LinkedIn, Newsletter, Mailing, Event, Video, Slides
├── 05 Departements/Vente/                 ce qui convertit : Listes-prospection, Messages, Propositions, Pipeline
├── 05 Departements/Strategie/             réflexions de dirigeant
├── 11 Archives/              terminé ou inactif
└── .claude/skills/        les skills transverses
```

Chaque dossier de travail (`06 Clients/`, `05 Departements/Contenu/`, `05 Departements/Vente/`, `05 Departements/Strategie/` et leurs sous-dossiers) contient une **note du même nom que le dossier** (`05 Departements/Vente/Vente.md`) qui dit ce qui va dedans et où on en est, plus un `_log.md` écrit par `/done`. Les livrables vont dans `livrables/`, ce qu'on reçoit dans `sources/`, les assets propres au dossier dans `ressources/`.

**Principes structurants :**

1. **Divulgation progressive** : `CLAUDE.md` et `01 About-Me/` sont chargés à chaque session. `02 Contexte/` n'est chargé que si la tâche le justifie. La note d'un dossier se lit avant de travailler dedans.
...
> **Note sur le naming** : les fichiers de `02 Contexte/` portent des noms en anglais (`Constitution-Identity.md`, `Goals-and-Direction.md`, etc.). Ce sont les noms de référence des prompts d'interview. Vous pouvez les renommer en français si vous préférez : pensez juste à mettre à jour les prompts en conséquence.
...
```
Vérifie que la structure de Superfounder OS est intacte : 01 About-Me/, 02 Contexte/ (7 fichiers),
03 Branding/, 08 Ressources/, 00 Inbox/, 09 Journal/, 08 Ressources/Veille/, 07 Meeting/, 06 Clients/, 05 Departements/Contenu/,
05 Departements/Vente/, 05 Departements/Strategie/, 11 Archives/, et .claude/skills/. Confirme avec un `ls`.
```

**Validation :** Les dossiers sont là, les fichiers de `01 About-Me/` et `02 Contexte/` contiennent `[à remplir]`. On va les remplir un par un.
...
2. **Fichiers à lire au démarrage de chaque session**
   - Liste des fichiers 01 About-Me/* à charger systématiquement
...
5. **Routage automatique du contexte**
   - Quels fichiers 02 Contexte/* charger selon la tâche
...
## 4. Étape 2 : Construire `01 About-Me/`
...
Écris-le directement dans 01 About-Me/about-me.md.
```
...
Écris dans 01 About-Me/my-company.md.
```
...
```
Crée le fichier 01 About-Me/anti-ai-voice.md.
...
## 5. Étape 3 : Construire `02 Contexte/` (les 7 fichiers stratégiques)
...
## 6. Étape 4 : Peupler `08 Ressources/`, `08 Ressources/Veille/` et les dossiers de travail
...
### 6.1 `08 Ressources/` (ce qui aide à produire)

**Objectif :** stocker tout ce qui est réutilisable par n'importe quel dossier : templates, listes de hooks, exemples de bons mails, docs d'outils. `08 Ressources/templates/` contient déjà les gabarits de note de dossier et de journal.
...
### 6.2 `08 Ressources/Veille/`, `09 Journal/`, `07 Meeting/` (ce qui entre et ce qui s'est passé)

- `08 Ressources/Veille/sources/` reçoit le brut : articles clippés, transcripts de vidéos, newsletters. On ne le modifie jamais. `08 Ressources/Veille/wiki/` reçoit la connaissance distillée, écrite par Claude via `/notes-permanentes`. `08 Ressources/Veille/Veille.md` porte le schéma du wiki.
- `09 Journal/` reçoit un fichier par jour, écrit par `/done` à la fin de chaque session, et un fichier par semaine, écrit par `/weekly-review`. Vous ne l'écrivez pas à la main.
- `07 Meeting/` reçoit vos transcripts de calls (tldv, Granola, Claap). Ce qui s'y décide ruisselle vers la note du client concerné.

#### [HUMAIN] Une première source

Déposez dans `08 Ressources/Veille/sources/` un article que vous citez souvent, et lancez `/notes-permanentes` : c'est la première page de votre wiki.

### 6.3 Les dossiers de travail

Quatre dossiers de travail sont livrés : `06 Clients/` (un dossier par client et par produit), `05 Departements/Contenu/` (LinkedIn, Newsletter, Mailing, Event, Video, Slides), `05 Departements/Vente/` (Listes-prospection, Messages, Propositions, Pipeline), `05 Departements/Strategie/`. Chacun a sa note du même nom, qui dit ce qui va dedans et ce qui n'y va pas.

Trois sous-dossiers reviennent partout :
- `sources/` = matière brute reçue (transcripts, briefs, exports)
- `livrables/` = ce qui est produit
- `ressources/` = assets propres à ce dossier (à distinguer de `08 Ressources/` à la racine, transversal)

#### [HUMAIN] Adaptez à votre activité

Si votre métier appelle un dossier de plus (`Formation/`, `Recrutement/`, `Production/`), créez-le sur le même patron : une note du même nom depuis `08 Ressources/templates/departement.md`, un `_log.md`, et `sources/ livrables/ ressources/`. Une initiative datée (un event, un lancement) vit en sous-dossier du dossier qu'elle sert, avec sa propre note, et part dans `11 Archives/` une fois finie.
...
Exemples :
- `Propale-Acme-Refonte_2026-05-04.docx` dans `05 Departements/Vente/Propositions/`
- `Post-Defi-IA-20j_2026-05-04.md` dans `05 Departements/Contenu/LinkedIn/`
- `Edito-Newsletter-12_2026-05-04.md` dans `05 Departements/Contenu/Newsletter/`
...
Un **skill** est un workflow réutilisable que Claude peut déclencher automatiquement. Il vit dans `10 Skills/`.
...
- Crée 10 Skills/[nom-skill]/SKILL.md
...
|---|---|
| `01 About-Me/about-me.md` | Tous les 6-12 mois (rare changement) |
| `01 About-Me/my-company.md` | Tous les 1-3 mois (snapshot) |
| `02 Contexte/Goals-and-Direction.md` | Tous les 30 jours |
| `02 Contexte/Life-and-Work-Reality.md` | Tous les 3 mois |
| `02 Contexte/Offer-Positioning.md` | À chaque nouvelle offre / changement de prix |
| `02 Contexte/Clients-Problems-and-Messages.md` | Tous les 3 mois (alimenter avec nouveaux verbatim) |
| `02 Contexte/Expertise-Standards-and-Landmines.md` | Tous les 6 mois ou après une grosse erreur |
| `02 Contexte/Tone-and-Voice.md` | Tous les 6 mois ou après évolution de style |
| `02 Contexte/Constitution-Identity.md` | Tous les 12 mois (très stable) |
...
3. Noter dans `08 Ressources/Veille/veille/` tout ce qui a marqué la semaine
...
3. Lire les Daily Logs du mois et en faire un résumé dans `08 Ressources/Veille/`
4. Identifier 1 nouvelle tâche récurrente → candidate à un skill

### 9.4 Rituel trimestriel (1h)

Tous les 3 mois :
1. Revue complète des fichiers `02 Contexte/`
...
- [ ] `01 About-Me/about-me.md` rédigé via interview 3-prompts
- [ ] `01 About-Me/my-company.md` rédigé via interview 3-prompts
- [ ] `01 About-Me/anti-ai-voice.md` créé (optionnel mais recommandé si contenu)

### Niveau 2 : Cerveau (chargé à la demande)
- [ ] `02 Contexte/Constitution-Identity.md`
- [ ] `02 Contexte/Life-and-Work-Reality.md`
- [ ] `02 Contexte/Goals-and-Direction.md`
- [ ] `02 Contexte/Offer-Positioning.md`
- [ ] `02 Contexte/Clients-Problems-and-Messages.md` ⭐
- [ ] `02 Contexte/Expertise-Standards-and-Landmines.md`
- [ ] `02 Contexte/Tone-and-Voice.md`

### Niveau 3 : Workspace opérationnel
- [ ] Dossiers de travail adaptés à mon activité, chacun avec sa note
- [ ] `08 Ressources/templates/` initié (au moins fichiers vides)
- [ ] `08 Ressources/Veille/` initié avec template Daily Log
...
4. **Partage d'équipe** : étendre le sandbox à votre équipe (avec ses propres `01 About-Me/` par membre)
...
3. `setup-claude-infrastructure-about-me` : Étape 4 (01 About-Me/)
...
**Sources canoniques pour les prompts d'interview 02 Contexte/ :**
...
*Guide v2 : 2026-05-04*
*Sources : "Construisez votre infrastructure" + "Créez votre cerveau IA" (Hugo Dollfus, Substack) + page Notion "Votre Cerveau IA" (7 sous-pages avec les prompts originaux des fichiers 02 Contexte/)*
