...
3. Couche CRM : ajoutez les fermés-perdus de plus de 6 mois (ils avaient le problème, le moment était mauvais), les perdus face à un concurrent dont l'offre a comblé le manque, les churnés de plus d'un an, les anciens contacts jamais convertis. Ces comptes sont chauds : ils connaissent déjà l'utilisateur.
...
7. Scorez et rangez avec `qualifier_liste` : tier A (10 à 50 comptes, sur mesure, 3 à 5 contacts, plusieurs canaux), tier B (50 à 200, personnalisation par segment, 2 à 3 contacts), tier C (200 à 1 000, séquence standard, 1 contact). Un compte sans fit prouvé sort, même s'il est prestigieux.
...
|---|---|---|---|---|
| 1 et 3 | lire_crm (affaires gagnées, puis perdues) | `crm` (`lire --statut gagnes`, puis `perdus`) | rien | `entreprise`, `domaine`, `secteur`, `effectif`, `source = crm`, raison et date de la perte dans `signal_detail` et `signal_date` |
| 4 | `trouver_lookalikes` | `trouver-lookalikes` | `domaine` des clients de référence, 10 par appel, plusieurs appels au-delà | `entreprise`, `domaine`, `linkedin_entreprise_url`, `secteur`, `effectif`, `pays`, `source = lookalikes`, `date_extraction` |
...
| Tier A | 10 à 50 comptes, 3 à 5 contacts, effort maximal |
| Tier B | 50 à 200 comptes, 2 à 3 contacts, personnalisation par segment |
...
- "Qui dans mon CRM je peux relancer ?" : export des fermés-perdus de plus de 6 mois, des churnés et des contacts sans affaire ; réponse attendue : une liste chaude avec la raison de la perte et la date, classée par tier.
