...
|---|---|---|---|
| Chaudes | des gens ou des entreprises qui ont déjà un lien avec l'utilisateur : réactions à ses posts, inscrits à sa newsletter, participants à son événement, fermés-perdus et churnés du CRM | la plus forte | faible, mais chaque ligne vaut dix lignes froides |
| Bases larges | recherche par critères dans une base : Sales Navigator, actor Apollo, Crustdata | aucune, c'est le fit qui compte | fort |
...
|---|---|---|---|---|---|
| Beaucoup d'entreprises par secteur, effectif, zone | base large | `trouver_entreprises` | Apify, actor base large de `05_Departements/Go-to-Market/OUTILS.md` (`code_crafter/leads-finder` ou son remplaçant) ; Crustdata en priorité api | milliers | entreprise, domaine, secteur, effectif, ville, pays |
...
| Ceux que l'utilisateur connaît déjà | CRM | `dedoublonner` | HubSpot, fermés-perdus de plus de 6 mois, churnés, contacts sans affaire | dizaines à centaines | tout le contact, avec son historique |
...
Chaque outil écrit les colonnes de `CONVENTIONS.md` section 8, avec `source` (le nom de la source, par exemple `google-maps`, `sales-navigator`, `actor-apollo`, `lookalikes`, `crm`) et `date_extraction`. Les actors nommés ici sont ceux de `05_Departements/Go-to-Market/OUTILS.md` au moment de l'écriture ; si `05_Departements/Go-to-Market/OUTILS.md` en nomme un autre pour le même verbe, c'est lui qui gagne, sans rien changer à la méthode.

## Choisir selon la cible

| Cible de l'utilisateur | Première source | Seconde source | Pourquoi |
|---|---|---|---|
| SaaS, tech, services numériques en France | Sales Navigator via Unipile | actor Apollo | données LinkedIn à jour, l'actor apporte le volume et les domaines |
| PME traditionnelles (industrie, BTP, cabinets, distribution) | actor Apollo ou Crustdata | annuaires professionnels | ces entreprises sont peu actives sur LinkedIn, l'annuaire les liste toutes |
...
Demande : "des cabinets d'expertise comptable de 10 à 50 personnes en Île-de-France". Plan : actor Apollo (secteur comptabilité, effectif 11-50, région), puis annuaire de l'ordre régional pour les absents, fusion, dédoublonnage par domaine. Ordre de grandeur, à confirmer par un test de volume : 800 à 1 500 entreprises avant qualification.
