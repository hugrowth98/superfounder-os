...
7. Une liste vit. Les emails meurent à 2 % par mois, les titres à 3 % par mois. On relance `trouver_email` sur ce qui a plus de 30 jours, on ré-enrichit ce qui a plus de 90 jours, on relit l'ICP chaque trimestre avec ce qui a signé.

## Ce qui fait une bonne liste

Par ordre de valeur, ce qu'une ligne doit avoir :

1. Un email vérifié (`email_statut = valide`) ou un `linkedin_url` joignable.
...
| Pas de vérification | bounces, domaine grillé | `trouver_email` en mode statut sur 100 % des lignes avant envoi |
...
- [ ] 100 % des emails ont un `email_statut`, bounce attendu sous 1 %
- [ ] au moins 70 % des lignes ont un email `valide` ou un `linkedin_url`
...
- [ ] la liste de suppression a été croisée
- [ ] le fichier est nommé `<verbe>_<sujet>_<date>.csv`, colonnes de `CONVENTIONS.md` section 8, dans `05_Departements/Go-to-Market/Listes-prospection/`
