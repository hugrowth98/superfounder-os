...
3. **Dérouler la ligne rouge** dans l'ordre du guide : domaines (jamais le principal, plusieurs registrars, renouvellement automatique, redirection 301), espaces de travail (un par domaine, deux boîtes à vrais prénoms, photo), DNS (MX, SPF unique, DKIM 2048, DMARC à la main), connexion à Lemlist, chauffe 3 semaines, lancement à 10 à 15 par jour, montée de 20 % par semaine.
4. **Vérifier avant tout envoi** : `check_domain_health`, `get_domain_dns`, score de chauffe au-dessus de 70 %, `run_inbox_placement_test` sur le texte final, liste vérifiée à 100 %.
...
Sortie : le tableau de suivi des domaines (section 10 du guide) dans `05_Departements/Go-to-Market/infra-email_<YYYY-MM-DD>.md`, et un plan daté.
...
| Chauffe | 3 semaines minimum, jamais coupée, santé > 70 % (cible 90 %) |
| Âge du domaine avant envoi | 4 semaines minimum, 6 à 8 idéal |
| Montée | 20 % par semaine au plus, un nouveau domaine par semaine au plus |
| Rebond | < 2 % cible, pause à 3 à 5 %, stop à plus de 5 % |
| Plainte | < 0,1 % |
...
Semaine 2   Connecter les 4 boîtes à Lemlist, activer la chauffe (10 à 15 par jour, réponse 30 à 40 %).
            Aucun envoi.
Semaine 3   Chauffe. Vérifier la santé chaque jour (cible 90 %).
            Écrire et relire la séquence, vérifier la liste à 100 %.
Semaine 4   Premier lot : 50 contacts, 10 à 15 emails par boîte et par jour. Surveiller 3 jours.
Semaines 5 à 8   +20 % par semaine jusqu'à 25 par boîte. Audit de délivrabilité chaque lundi.
Semaine 9   30 par boîte. Troisième domaine si le marché le demande.
```
...
3. Chauffe de 3 semaines avant le premier envoi, jamais coupée ensuite.
...
- "Monte-moi mon infra de zéro" : dimensionnement, plan daté sur 9 semaines, étapes données une par une (domaines, espaces de travail, DNS, connexion, chauffe), vérifications Lemlist à chaque étape, tableau de suivi créé.
