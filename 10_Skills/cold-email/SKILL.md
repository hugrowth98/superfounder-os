...
| Réactiver d'anciens prospects | `reengagement` | "réengager", "réactiver", "closed-lost", "affaire perdue", "il a disparu", "no-show", "plus de nouvelles depuis" | `{SKILL_BASE}/sous-skills/reengagement/SKILL.md` |
...
| Répondre à une réponse | le master | "il m'a répondu", "comment je réponds", "il veut de la doc", "il dit que", "pas le bon contact" | `{SKILL_BASE}/ressources/reponses.md` |
...
| Séquence complète multicanal | le master, puis `premier-contact` et `relance` | "séquence complète", "7 touches", "email et LinkedIn", "campagne lead magnet" | `{SKILL_BASE}/ressources/sequences.md` |
| Brief avant campagne | le master | "brief", "on part sur quel angle", "je vise qui" | `{SKILL_BASE}/ressources/brief-strategie.md` |
...
3. **La demande précise.** Un objet seul : `objets`. Une ouverture, une personnalisation, des prompts : `personnalisation`. Une réponse à traiter : `reponses.md`. Un message LinkedIn : `linkedin-messages.md`.
...
- **Les réponses** (`{SKILL_BASE}/ressources/reponses.md`) : intéressé, doc demandée, pas maintenant, pas le bon contact, objection, pas intéressé, no-show, leads entrants.
...
- **Les séquences prêtes** (`{SKILL_BASE}/ressources/sequences.md`) : structures 2, 3 et 4 emails, la séquence 7 touches "le plafond de la recommandation", la séquence 2 emails "lead magnet", timing et saisonnalité.
...
- **Les règles transverses** : 50 à 90 mots ; texte brut ; un CTA ; fait avant adjectif ; vouvoiement ; aucun tiret cadratin ; 100 % des emails vérifiés ; la rotation des angles entre emails ; le signal avant le froid ; trois exemples validés avant `envoyer_sequence`.
...
| Taux de réponse | 6 à 8 % à froid, 18 à 22 % sur signal, 35 à 40 % sur signaux empilés |
...
| Séquence | 3 emails : J0, J+3 même fil, J+14 nouvel objet ; puis 3 mois de pause |
...
| Rebond, plainte | rebond sous 2 %, plainte sous 0,1 % ; stop à 5 % de rebond |
...
Ces chiffres viennent de campagnes anglophones à volume ; remplacez-les par ceux de l'utilisateur dès 500 envois.
