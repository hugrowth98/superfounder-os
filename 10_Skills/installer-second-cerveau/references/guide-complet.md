...
├── 01_About-Me/              qui vous êtes (chargé à chaque session)
│   ├── about-me.md
│   ├── my-company.md
│   └── anti-ai-voice.md
├── 02_Contexte/              votre cerveau (chargé à la demande)
...
├── 08_Ressources/            templates, exemples, docs d'outils
├── 00_Inbox/                 capture en vrac
├── 09_Journal/               un fichier par jour, écrit par /done
├── 08_Ressources/Veille/                sources brutes et wiki de connaissance
├── 07_Meeting/               transcripts de calls
├── 06_Clients/        un dossier par client et par produit
├── 05_Departements/Marketing/             ce qui fait venir : LinkedIn, Newsletter, Mailing, Event, Video, Slides
├── 05_Departements/Vente/                 ce qui convertit : Listes-prospection, Messages, Propositions, Pipeline
├── 05_Departements/Strategie/             réflexions de dirigeant
├── 11_Archives/              terminé ou inactif
└── .claude/skills/        les skills transverses
```

Chaque dossier de travail (`06_Clients/`, `05_Departements/Marketing/`, `05_Departements/Vente/`, `05_Departements/Strategie/` et leurs sous-dossiers) contient une **note du même nom que le dossier** (`05_Departements/Vente/Vente.md`) qui dit ce qui va dedans et où on en est, plus un `_log.md` écrit par `/done`. Les livrables vont dans `livrables/`, ce qu'on reçoit dans `sources/`, les assets propres au dossier dans `ressources/`.

**Principes structurants :**

1. **Divulgation progressive** : `CLAUDE.md` et `01_About-Me/` sont chargés à chaque session. `02_Contexte/` n'est chargé que si la tâche le justifie. La note d'un dossier se lit avant de travailler dedans.
...
> **Note sur le naming** : les fichiers de `02_Contexte/` portent des noms en anglais (`Constitution-Identity.md`, `Goals-and-Direction.md`, etc.). Ce sont les noms de référence des prompts d'interview. Vous pouvez les renommer en français si vous préférez : pensez juste à mettre à jour les prompts en conséquence.
...
```
Vérifie que la structure de Superfounder OS est intacte : 01_About-Me/, 02_Contexte/ (7 fichiers),
03_Branding/, 08_Ressources/, 00_Inbox/, 09_Journal/, 08_Ressources/Veille/, 07_Meeting/, 06_Clients/, 05_Departements/Marketing/,
05_Departements/Vente/, 05_Departements/Strategie/, 11_Archives/, et .claude/skills/. Confirme avec un `ls`.
```

**Validation :** Les dossiers sont là, les fichiers de `01_About-Me/` et `02_Contexte/` contiennent `[à remplir]`. On va les remplir un par un.
...
2. **Fichiers à lire au démarrage de chaque session**
   - Liste des fichiers 01_About-Me/* à charger systématiquement
...
5. **Routage automatique du contexte**
   - Quels fichiers 02_Contexte/* charger selon la tâche
...
## 4. Étape 2 : Construire `01_About-Me/`
...
Écris-le directement dans 01_About-Me/about-me.md.
```
...
Écris dans 01_About-Me/my-company.md.
```
...
```
Crée le fichier 01_About-Me/anti-ai-voice.md.
...
## 5. Étape 3 : Construire `02_Contexte/` (les 7 fichiers stratégiques)
...
## 6. Étape 4 : Peupler `08_Ressources/`, `08_Ressources/Veille/` et les dossiers de travail
...
### 6.1 `08_Ressources/` (ce qui aide à produire)

**Objectif :** stocker tout ce qui est réutilisable par n'importe quel dossier : templates, listes de hooks, exemples de bons mails, docs d'outils. `08_Ressources/templates/` contient déjà les gabarits de note de dossier et de journal.
...
### 6.2 `08_Ressources/Veille/`, `09_Journal/`, `07_Meeting/` (ce qui entre et ce qui s'est passé)

- `08_Ressources/Veille/sources/` reçoit le brut : articles clippés, transcripts de vidéos, newsletters. On ne le modifie jamais. `08_Ressources/Veille/wiki/` reçoit la connaissance distillée, écrite par Claude via `/notes-permanentes`. `08_Ressources/Veille/Veille.md` porte le schéma du wiki.
- `09_Journal/` reçoit un fichier par jour, écrit par `/done` à la fin de chaque session, et un fichier par semaine, écrit par `/weekly-review`. Vous ne l'écrivez pas à la main.
- `07_Meeting/` reçoit vos transcripts de calls (tldv, Granola, Claap). Ce qui s'y décide ruisselle vers la note du client concerné.

#### [HUMAIN] Une première source

Déposez dans `08_Ressources/Veille/sources/` un article que vous citez souvent, et lancez `/notes-permanentes` : c'est la première page de votre wiki.

### 6.3 Les dossiers de travail

Quatre dossiers de travail sont livrés : `06_Clients/` (un dossier par client et par produit), `05_Departements/Marketing/` (LinkedIn, Newsletter, Mailing, Event, Video, Slides), `05_Departements/Vente/` (Listes-prospection, Messages, Propositions, Pipeline), `05_Departements/Strategie/`. Chacun a sa note du même nom, qui dit ce qui va dedans et ce qui n'y va pas.

Trois sous-dossiers reviennent partout :
- `sources/` = matière brute reçue (transcripts, briefs, exports)
- `livrables/` = ce qui est produit
- `ressources/` = assets propres à ce dossier (à distinguer de `08_Ressources/` à la racine, transversal)

#### [HUMAIN] Adaptez à votre activité

Si votre métier appelle un dossier de plus (`Formation/`, `Recrutement/`, `Production/`), créez-le sur le même patron : une note du même nom depuis `08_Ressources/templates/departement.md`, un `_log.md`, et `sources/ livrables/ ressources/`. Une initiative datée (un event, un lancement) vit en sous-dossier du dossier qu'elle sert, avec sa propre note, et part dans `11_Archives/` une fois finie.
...
Exemples :
- `Propale-Acme-Refonte_2026-05-04.docx` dans `05_Departements/Vente/Propositions/`
- `Post-Defi-IA-20j_2026-05-04.md` dans `05_Departements/Marketing/LinkedIn/`
- `Edito-Newsletter-12_2026-05-04.md` dans `05_Departements/Marketing/Newsletter/`
...
Un **skill** est un workflow réutilisable que Claude peut déclencher automatiquement. Il vit dans `10_Skills/`.
...
- Crée 10_Skills/[nom-skill]/SKILL.md
...
|---|---|
| `01_About-Me/about-me.md` | Tous les 6-12 mois (rare changement) |
| `01_About-Me/my-company.md` | Tous les 1-3 mois (snapshot) |
| `02_Contexte/Goals-and-Direction.md` | Tous les 30 jours |
| `02_Contexte/Life-and-Work-Reality.md` | Tous les 3 mois |
| `02_Contexte/Offer-Positioning.md` | À chaque nouvelle offre / changement de prix |
| `02_Contexte/Clients-Problems-and-Messages.md` | Tous les 3 mois (alimenter avec nouveaux verbatim) |
| `02_Contexte/Expertise-Standards-and-Landmines.md` | Tous les 6 mois ou après une grosse erreur |
| `02_Contexte/Tone-and-Voice.md` | Tous les 6 mois ou après évolution de style |
| `02_Contexte/Constitution-Identity.md` | Tous les 12 mois (très stable) |
...
3. Noter dans `08_Ressources/Veille/veille/` tout ce qui a marqué la semaine
...
3. Lire les Daily Logs du mois et en faire un résumé dans `08_Ressources/Veille/`
4. Identifier 1 nouvelle tâche récurrente → candidate à un skill

### 9.4 Rituel trimestriel (1h)

Tous les 3 mois :
1. Revue complète des fichiers `02_Contexte/`
...
- [ ] `01_About-Me/about-me.md` rédigé via interview 3-prompts
- [ ] `01_About-Me/my-company.md` rédigé via interview 3-prompts
- [ ] `01_About-Me/anti-ai-voice.md` créé (optionnel mais recommandé si contenu)

### Niveau 2 : Cerveau (chargé à la demande)
- [ ] `02_Contexte/Constitution-Identity.md`
- [ ] `02_Contexte/Life-and-Work-Reality.md`
- [ ] `02_Contexte/Goals-and-Direction.md`
- [ ] `02_Contexte/Offer-Positioning.md`
- [ ] `02_Contexte/Clients-Problems-and-Messages.md` ⭐
- [ ] `02_Contexte/Expertise-Standards-and-Landmines.md`
- [ ] `02_Contexte/Tone-and-Voice.md`

### Niveau 3 : Workspace opérationnel
- [ ] Dossiers de travail adaptés à mon activité, chacun avec sa note
- [ ] `08_Ressources/templates/` initié (au moins fichiers vides)
- [ ] `08_Ressources/Veille/` initié avec template Daily Log
...
4. **Partage d'équipe** : étendre le sandbox à votre équipe (avec ses propres `01_About-Me/` par membre)
...
3. `setup-claude-infrastructure-about-me` : Étape 4 (01_About-Me/)
...
**Sources canoniques pour les prompts d'interview 02_Contexte/ :**
...
*Guide v2 : 2026-05-04*
*Sources : "Construisez votre infrastructure" + "Créez votre cerveau IA" (Hugo Dollfus, Substack) + page Notion "Votre Cerveau IA" (7 sous-pages avec les prompts originaux des fichiers 02_Contexte/)*
