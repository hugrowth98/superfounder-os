...
description: >
  Détecte les personnes qui réagissent à votre contenu (likes et commentaires sur vos posts, participants à vos webinars, abonnés de votre newsletter exportés de votre outil ; les abonnés d'une page ou d'un profil LinkedIn ne se listent pas, on passe par les engageurs) et déclenche l'approche dans les 24 à 48 heures, sur le sujet du contenu. Se déclenche sur : "qui a liké", "qui a commenté", "engagement sur mes posts", "mes abonnés", "nouveaux followers", "participants du webinar", "inscrits à ma newsletter", "lead magnet", "commentaires sous mon post". Ne pas utiliser pour : l'engagement avec les posts d'un concurrent ou d'un influenceur, voir `signaux-concurrents` et la famille 5 de la taxonomie ; les visiteurs de votre site, non couverts.
---

Quelqu'un qui commente votre post, vient à votre webinar ou s'abonne à votre newsletter vous connaît déjà et parle de votre sujet cette semaine : taux de réponse de 25 à 30 %, un commentaire vaut 35 points, un like 25, et la fenêtre se ferme en 7 jours.

## Ressources

- `{SKILL_BASE}/ressources/bareme-signaux.md` : commentaire 35, like 25, webinar 25, engagement avec un collaborateur 20, abonnement à la newsletter 15.
...
2. **Détecter** (Exécution). Posts : chaque jour ou chaque semaine selon votre rythme de publication. Webinar : la liste de présence le jour même. Newsletter : les nouveaux abonnés, exportés de votre outil.
3. **Classer par force.** Commentaire, puis participant à un webinar, puis like, puis abonné à la newsletter. Un commentateur vaut deux likers ; un participant vaut deux inscrits.
...
5. **Dédoublonner et cumuler.** La même personne qui like trois posts en un mois vaut plus qu'un like isolé : chaque réaction garde son multiplicateur de fraîcheur, puis on additionne.
...
|---|---|---|---|
| 1a | scraper_engagement | `scraper-engagement` | Unipile : `--mes-posts 5` pour vos derniers posts, `--post <url>` pour un post précis, `--mes-posts 5 --posts-de <slug>` pour un collaborateur qui publie. Inclus dans l'abonnement, dans les quotas LinkedIn. Secours : `harvestapi/linkedin-post-comments`, 0,002 $ par commentaire. |
| 1b | import | interne (Claude) | webinar, événement, newsletter : votre export CSV (inscrits, présents, abonnés), colonnes ramenées aux noms normalisés |
...
**CSV en entrée** : la liste des URL de posts à surveiller ; ou votre export d'inscrits et de présents ; ou rien avec `scraper-engagement --mes-posts 5`.

**CSV en sortie** : colonnes normalisées, plus `signal_type` (`commentaire`, `like` ; pour les imports à la main, hors script : `webinar_present`, `webinar_inscrit`, `newsletter_abonne`, `abonne`), `signal_date`, `signal_detail` ("commentaire sous « {{titre du post}} » du {{date}} : {{extrait}}" ; "like sur 3 posts en 30 jours"), `score_signal`, `fraicheur`, `source` (`unipile` ; vide pour un import à la main).
...
| Abonné à votre newsletter (export de votre outil), abonné à votre page ou profil relevé à la main | 15 | j0 à j2 | sous 48 h, jamais seul |
...
| Empilement type | commentaire (35×1,2) + like sur un autre post (25×1,2) + abonnement à votre newsletter (15×1,2), chacun avec son multiplicateur, tous dans la semaine = 90, Tiède ; plus une offre d'emploi ouverte depuis 18 jours (40×1,5) = 150, Brûlant |
...
- "Cette personne a liké trois de mes posts ce mois-ci" : chaque like garde son multiplicateur (par exemple j2, j10 et j20 : 25×1,2 + 25×1,0 + 25×0,7 = 73, Tiède) ; on vérifie ses autres signaux (poste, offres, levée) ; message sur le sujet commun aux trois posts, sans mentionner les likes.
