...
4. **Dater.** `signal_date` = date de prise de poste (le profil la donne au mois près ; le signal la donne au jour). `fraicheur` = jours écoulés. Avant j14 : on attend, sauf le champion à qui on écrit le jour même. j14 à j45 : pic. j46 à j90 : encore bon. Après j90 : contexte.
...
|---|---|---|---|
| 1 | detecter_signal | `detecter-signaux` (script `detecter_signal.py`) | actor `signalbase/signalbase-api`, `signalType: job-changes`. Mode champions : `personLinkedinUrl`, un appel par personne. Mode compte cible : `companyLinkedinUrl`. Mode marché : `countries: FR`, `seniorities: c_level,vp,director,head`, `departments` et `positions` selon vos personas, `date_preset: last_30d`. 0,04 $ par résultat. |
| 2 | qualifier_liste | `qualifier-liste` | score ICP de l'entreprise d'arrivée, exclusions (concurrents, clients actuels) |
| 3 | enrichir_personne | `enrichir-personne` | profil complet : date de prise de poste confirmée, ancien employeur, ancien titre |
...
| après j90 | ×0,7 | priorité basse, ses fournisseurs sont choisis |
...
- Jamais "j'ai vu que vous aviez changé de poste" ni "félicitations pour votre nomination". Le poste s'évoque par ce qu'il implique, pas par l'annonce.
...
- "Préviens-moi quand un de mes anciens clients change de boîte" : mode champions, CSV de vos anciens clients avec `linkedin_url`, detecter_signal chaque jour, message le jour même sur ce qu'on avait fait ensemble.
- "Qui vient d'être nommé directeur commercial dans une PME française ce mois-ci ?" : mode marché, `positions: vp of sales`, `seniorities: head,director,vp`, `countries: FR`, `date_preset: last_30d`, qualification ICP, contact programmé à j14.
