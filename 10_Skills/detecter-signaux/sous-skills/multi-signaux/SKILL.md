...
5. **Regrouper par compte** (`domaine`) : deux personnes ou plus avec un signal dans les 30 jours, on ajoute 70 au compte et on le traite comme une seule affaire (`tier` = `comite`).
...
**CSV en sortie** : une ligne par personne, colonnes normalisées, `signal_type` cumulé, `signal_detail` cumulé, `signal_date` (la plus récente), `fraicheur`, `score_signal` (total), `tier` (`brulant`, `chaud`, `tiede`, `frais`, `froid`, ou `comite`), plus deux colonnes de travail : `action` et `avant_le` (date limite de contact). Nom : `multi-signaux_<sujet>_<date>.csv`.
...
| Tier 3 seul (actualité, abonné, ouverture) | Frais | nurture |
...
| Fraîcheur contre nombre | un signal Tier 1 d'hier (75×1,5 = 113, Chaud) bat trois signaux Tier 2 de deux mois ((45+40+25)×0,3 = 33, Frais) |
