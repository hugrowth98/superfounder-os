...
`--montant-min` en dollars (le script convertit en cents pour `amount_min`), `positions`, `seniorities`,
...
`05_Departements/Go-to-Market/Signaux/detecter-signaux_<type>-<source>-<sujet>_<date>.csv`, une ligne par signal :
`entreprise`, `domaine`, `linkedin_entreprise_url`, `ville`, `pays`, `secteur`, `effectif`,
`source` (`signalbase/signalbase-api`), `date_extraction`, `signal_type` (`levee`, `acquisition`, `offre_emploi`, `changement_poste`, `investisseur` ou `profil_entreprise` selon le type), `signal_date`,
...
4. Liste suivie : `--liste-suivie 05_Departements/Go-to-Market/Signaux/comptes-suivis.csv` garde seulement les signaux qui
   concernent ces comptes ou ces personnes (filtre après le run, coût du run complet). Pour des
   changements de poste sur peu de cibles, `--par-cible` lance une requête exacte par entreprise
   (`linkedin_entreprise_url`) et par personne (`linkedin_url`) de la liste, coût = cibles x limite x 0,04 $ : préférez-le sous 30 cibles.
...
6. Lien cliquable, nombre de signaux, un seul next step : `qualifier-liste` (score ICP et `tier`),
   puis le sous-skill `multi-signaux` pour `score_signal` et `chaleur` (calculés par Claude, pas par un script),
   puis `trouver-personnes` sur les comptes retenus.
...
- Un signal n'est pas une preuve : `verification` vaut `verified`, `unverified` ou `pending`
  (`--verification verified` pour ne garder que les confirmés), gardez la colonne et citez `url_source` dans un message plutôt que le montant.
...
| `--montant-min` sans effet | montant donné en cents | donner des dollars, le script convertit |
