...
description: >
  Jour 3 de Superfounder OS. Installe la machine de contenu LinkedIn : lit le second cerveau
  (Contexte/) pour ne rien redemander sur l'offre, la cible et la voix, puis construit avec
  l'utilisateur les trois fichiers de Projects/Contenu/ressources/ (strategie-contenu.md,
...
# Installer mon contenu (jour 3)

Tu installes la machine de contenu pour l'utilisateur. Il vient d'ouvrir `Projects/Contenu/`
...
|---|---|---|
| Offre, différenciation, preuves | `Contexte/Offer-Positioning.md` | jour 1 |
| Clients, problèmes, mots exacts | `Contexte/Clients-Problems-and-Messages.md` | jour 1 |
| Voix, marqueurs de style, ce qu'il ne dit jamais | `Contexte/Tone-and-Voice.md` | jour 1 (ou ce skill si vide) |
| Piliers, répartition, convictions, sources, créateurs admirés | `Projects/Contenu/ressources/strategie-contenu.md` | ce skill |
| Ses meilleurs posts | `Projects/Contenu/ressources/posts-de-reference.md` | ce skill |
| Posts des autres qui l'inspirent | `Projects/Contenu/ressources/swipe-file.md` | ce skill, puis au fil de l'eau |
...
Sans rien demander, lis à la racine du workspace : `Contexte/Offer-Positioning.md`,
`Contexte/Clients-Problems-and-Messages.md`, `Contexte/Tone-and-Voice.md`. Note lesquels
...
- **`Tone-and-Voice.md` est vide** : tu le construiras à la Phase 1 à partir de ses posts,
  c'est la meilleure source possible. Après validation, tu l'écris dans `Contexte/` (la
...
À partir des posts :
1. Range-les par type dans `Projects/Contenu/ressources/posts-de-reference.md` (Educational,
...
2. Déduis la synthèse de fin de fichier : marqueurs visuels, structure typique, longueurs,
   sujets à éviter.
3. Si `Contexte/Tone-and-Voice.md` était vide : déduis le ton, les marqueurs de style, le
...
Remplis `Projects/Contenu/ressources/strategie-contenu.md` section par section. Déduis un
maximum des posts et de `Contexte/`, puis pose UNE question à la fois sur ce qui manque, en
...
- Tu écris dans `Contexte/` uniquement si le fichier est encore `[à remplir]`, après validation.
- Jamais de donnée sur l'utilisateur dans un fichier de skill. Tout va dans `Contexte/` ou `ressources/`.
